...
<Steps>
  ...
  Add the [`istanbul`](/test) and [`canyon`](/test) plugins in [babel](https://babeljs.io/docs/config-files#configuration-file-types).
  ```js {3,4}
  module.exports = {
  ...
  <Callout>
    You can also specify them manually. For detailed configurations, please refer to[babel-plugin-canyon](/docs/guide/built-ins/tabs)。
  </Callout>
  ...
  <Callout type="info" emoji="ℹ️">
    Among them, Project ID, Branch, and SHA don't need to be configured. The canyon plugin will detect the pipeline environment variables. To view the supported CI platforms,[Ck](/docs/guide/built-ins/tabs).
  </Callout>
  ...
  UI automation will generate a large amount of coverage data. In order to reduce the amount of data transmission, you can choose to separate the hit and map data.[hit和map数据分离](/test)。
