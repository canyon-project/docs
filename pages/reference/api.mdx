...
| 参数名 | 类型 | 是否必须 | 说明 |
| --- | --- | --- | --- |
| projectID | string | 是 | git仓库ID，window.\_\_canyon_\_中 |
| sha | string | 是 | git提交ID，window.\_\_canyon_\_中 |
| branch | string | 否 | git分支，window.\_\_canyon_\_中 |
| instrumentCwd | string | 是 | babel插桩路径，window.\_\_canyon_\_中 |
| compareTarget | string | 否 | 对比目标，用作基线与当前sha对比计算变更行覆盖率 |
| coverage | Object | 是 | 覆盖率数据，window.\_\_coverage_\_ |
...
2. 需要加请求头 `Authorization: Bearer ${token}`，token为用户登录后获取的token，在 [Canyon平台](https://canyon.com/settings) 获取
:::
...
{
    "msg": "ok",
    "coverageId": "cltmheo883nrk14ij46q8lrxb",
